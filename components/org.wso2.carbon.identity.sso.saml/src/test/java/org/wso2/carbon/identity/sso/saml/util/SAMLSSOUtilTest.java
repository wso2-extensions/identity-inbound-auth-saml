...
public class SAMLSSOUtilTest extends PowerMockTestCase {
    ...
    private void prepareForGetSAMLSSOServiceProvider() throws Exception {
        ...
        samlssoServiceProviderDO.setIssuer(TestConstants.ISSUER_UNIQUE_ID);
        samlssoServiceProviderDO.setIssuerEntityValue(TestConstants.ISSUER_ENTITY_ID);
        samlssoServiceProviderDO.setIdpEntityIDAliasEnabled(true);
        ...
        ssoServiceProviderConfigManager.addServiceProvider(TestConstants.ISSUER_UNIQUE_ID, samlssoServiceProviderDO);
        when(ssoServiceProviderConfigManager.getServiceProvider(TestConstants.SP_ENTITY_ID)).thenReturn(samlssoServiceProviderDO);
    }
    ...
    public void testIsValidSAMLIssuer() throws Exception {
        ...
        boolean isSAMLIssuerValidated = SAMLSSOUtil.isValidSAMLIssuer(TestConstants.ISSUER_ENTITY_ID, TestConstants.ISSUER_UNIQUE_ID, MultitenantConstants.SUPER_TENANT_DOMAIN_NAME);
    ...
    public void testGetIssuerWhenEntityIDAliasEnabled() throws Exception {
        SAMLSSOUtil.setIssuerUniqueIdInThreadLocal(TestConstants.ISSUER_UNIQUE_ID);
        prepareForGetIssuer();
        prepareForGetSAMLSSOServiceProvider();
        TestUtils.startTenantFlow(MultitenantConstants.SUPER_TENANT_DOMAIN_NAME);
        Issuer issuer = SAMLSSOUtil.getIssuerFromTenantDomain(MultitenantConstants.SUPER_TENANT_DOMAIN_NAME);
