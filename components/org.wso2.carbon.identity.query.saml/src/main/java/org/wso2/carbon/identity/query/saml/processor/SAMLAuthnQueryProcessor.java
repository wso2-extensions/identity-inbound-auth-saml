...
public class SAMLAuthnQueryProcessor implements SAMLQueryProcessor {
    ...
    final static Log log = LogFactory.getLog(SAMLAuthnQueryProcessor.class);
    ...
    public Response process(RequestAbstractType request) {
        ...
        for (SAMLAssertionFinder finder : finders) {
            ...
            if (assertions.size() > 0 && assertions != null)
        ...
        if (assertionsMatchBySubject.size() > 0) {
            ...
            log.info("No Assertions Matched with Subject");
            return null;
        }
        ...
        if (uniqueAssertions.size() > 0) {
            try {
                ...
                log.info("SAMLAuthnQuery : response generated");
                return response;
